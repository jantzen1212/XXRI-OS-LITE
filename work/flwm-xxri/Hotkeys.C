...
#include <unistd.h>
#include <sys/wait.h>
// Ctrl+Alt+C toggles the XXRI Control Center.  Double-forked like flwm's own
// spawn so the window manager never collects a zombie.
static void ToggleControlCenter(void) {
  if (fork() == 0) {
    if (fork() == 0) {
      execl("/usr/local/bin/xxri-control-center",
            "xxri-control-center", "--toggle", (void*)0);
      _exit(1);
    }
    _exit(0);
  }
...
#endif
#ifdef XXRI
  {FL_CTRL+FL_ALT+'c',	ToggleControlCenter},
#endif
#ifdef EXTENDED_MOVEMENT_KEYS
