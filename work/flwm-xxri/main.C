...
#define program_version "Version 2.00"
...
" -cbg color\t\tCursor outline color", program_version
...
#else
  // FLTK 1.3 (what the target ships) has no per-screen scaling API; the
  // desktop runs at a fixed 75dpi, so the scale factor is simply 1.
  sf = 1.0f;
#endif
  TitleFontSz = (int) ((float)TITLE_FONT_SIZE * sf);
#ifdef XXRI
  // XXRI's bar height is a design constant, not a function of the font size.
  TitleSz     = (int) ((float)XXRI_TITLE_H * sf);
  ButtonSz    = TitleSz;
#else
...
#if FL_MAJOR_VERSION > 1 || (FL_MAJOR_VERSION == 1 && FL_MINOR_VERSION >= 4)
  for (int i = 0; i < Fl::screen_count(); i++) Fl::screen_scale(i, 1.0);
#endif
...
#ifdef XXRI
  // The desktop menu is the one piece of stock FLTK the user still sees, so
  // it gets the XXRI palette rather than the default gray bevels.
  // Fl::set_color(FL_BACKGROUND_COLOR) alone is not enough: FLTK boxes draw
  ...
  Fl::set_color(FL_SELECTION_COLOR,   (XXRI_MENU_SEL>>16)&0xff, (XXRI_MENU_SEL>>8)&0xff, XXRI_MENU_SEL&0xff);
#endif
  Fl_Root root;
  Root = &root;
