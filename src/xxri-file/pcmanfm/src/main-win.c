...
static void fm_main_win_init(FmMainWin *win)
{
    ...
    /* XXRI File ships its own icon; "folder" remains the fallback for a
     * theme that does not carry it. */
    if(gtk_icon_theme_has_icon(gtk_icon_theme_get_default(), "xxri-file"))
        gtk_window_set_icon_name(GTK_WINDOW(win), "xxri-file");
    else
    ...
    /* XXRI File: the mockup has neither a menubar nor a toolbar - the rail
     * carries navigation and the hamburger carries the menu.  Both widgets
     * are still built above so GtkUIManager keeps its accelerators and every
     * upstream action lookup ("/menubar/EditMenu/Cut", ...) still resolves;
     * they are simply never packed into the window. */
    gtk_widget_set_no_show_all(menubar, TRUE);
    gtk_widget_set_no_show_all(GTK_WIDGET(win->toolbar), TRUE);
    ...
    /* XXRI File: no status bar - the folder size lives in the location
     * pill and per-file detail lives in the list rows. */
    gtk_widget_set_no_show_all(GTK_WIDGET(win->statusbar), TRUE);
    ...
    /* ---- XXRI File shell -------------------------------------------
     * rail | content( header, notebook ).  vbox still owns the notebook so
     * upstream code that reaches for it keeps working unchanged. */
    xxri_ui_init_style();
    xxri_ui_enable_alpha(GTK_WIDGET(win));
    {
        ...
        win->xxri_rail = xxri_ui_build_rail(win);
        gtk_box_pack_start(GTK_BOX(hbox), win->xxri_rail, FALSE, FALSE, 0);

        gtk_style_context_add_class(gtk_widget_get_style_context(content),
                                    "xxri-file-content");
        /* The content column must carry NO outer margin.  It paints the
         * window surface, so a margin here leaves a gap between the rail and
         * the content through which the (transparent) window background - and
         * therefore the wallpaper - shows, making the file view look like a
         * separate floating widget parked beside the rail.  The mockup's
         * padding belongs INSIDE the column, on its children. */
        {
            GtkWidget* header = xxri_ui_build_header(win);

            gtk_widget_set_margin_start(header, 20);
            gtk_widget_set_margin_end(header, 20);
            gtk_widget_set_margin_top(header, 18);
            gtk_widget_set_margin_start(GTK_WIDGET(vbox), 20);
            gtk_widget_set_margin_end(GTK_WIDGET(vbox), 20);
            gtk_widget_set_margin_bottom(GTK_WIDGET(vbox), 14);
            gtk_box_pack_start(GTK_BOX(content), header, FALSE, FALSE, 0);
            gtk_box_pack_start(GTK_BOX(content), GTK_WIDGET(vbox), TRUE, TRUE, 0);
        }

        gtk_box_pack_start(GTK_BOX(hbox), content, TRUE, TRUE, 0);
        gtk_container_add(GTK_CONTAINER(win), xxri_ui_wrap_root(win, hbox));
    }
}
...
static void on_about(GtkAction* act, FmMainWin* win)
{
    if(!about_dlg)
    {
        GString *comments = g_string_new(_("File manager for XXRI OS Lite\n"));
...
#endif
        /* XXRI File is a fork of PCManFM.  The upstream author's credit
         * stays - it is his code, and the GPL requires the notice be kept. */
        g_string_append(comments, _("\nBased on PCManFM by Hon Jen Yee (PCMan)"));
        gtk_about_dialog_set_comments(about_dlg, comments->str);
        gtk_about_dialog_set_program_name(about_dlg, "XXRI File");
        gtk_about_dialog_set_logo_icon_name(about_dlg, "xxri-file");
...
static void on_tab_page_status_text(FmTabPage* page, guint type, const char* status_text, FmMainWin* win)
{
    if(page != win->current_page)
        return;
    /* XXRI File: refresh the location pill's size before the status-bar
     * early-out below - the pill is shown even though the bar is not. */
    if(type == FM_STATUS_TEXT_NORMAL)
    {
        char* sz = fm_tab_page_get_size_text(page);
        xxri_ui_update_size(win, sz);
        g_free(sz);
    }
    if (!app_config->show_statusbar)
        return;
...
static void on_tab_page_chdir(FmTabPage* page, FmPath* path, FmMainWin* win)
{
    ...
    xxri_ui_update_location(win, fm_tab_page_get_cwd(page));
}
