...
static void on_folder_start_loading(FmFolder* folder, FmTabPage* page)
{
    ...
    if(fm_folder_is_incremental(folder))
    {
        ...
        /* XXRI File hides /proc, /sys, /dev ... from ordinary listings.
         * add_filter only registers the callback - libfm does not refilter
         * until apply_filters() is called, so without this the filter is
         * silently inert. */
        fm_folder_model_add_filter(model, xxri_ui_model_filter, NULL);
...
/* XXRI File: the location pill shows the folder's total size on the right,
 * where upstream would have put an item count in the status bar.  Directories
 * are skipped - their on-disk inode size is meaningless to a user. */
char* fm_tab_page_get_size_text(FmTabPage* page)
{
    FmFolder* folder;
    FmFileInfoList* files;
    GList* l;
    ...
    if(!page || !page->folder_view)
        return NULL;
    folder = fm_folder_view_get_folder(page->folder_view);
    if(!folder)
        return NULL;
    files = fm_folder_get_files(folder);
    if(!files)
        return NULL;
    for(l = fm_file_info_list_peek_head_link(files); l; l = l->next)
    {
        ...
        if(fi && !fm_file_info_is_dir(fi))
            sum += fm_file_info_get_size(fi);
    }
    fm_file_size_to_str(buf, sizeof(buf), sum, fm_config->si_unit);
    return g_strdup(buf);
}
...
static void fm_tab_page_init(FmTabPage *page)
{
    ...
    /* XXRI File: Places live in the window rail (xxri-ui.c), so the libfm
     * side pane is built and kept - upstream code still talks to it, and the
     * View menu still switches its mode - but never shown. */
    gtk_widget_set_no_show_all(GTK_WIDGET(page->side_pane), TRUE);
    ...
    /* XXRI File: skin libfm's view to the XXRI surface. */
    xxri_ui_style_folder_view(GTK_WIDGET(folder_view));
