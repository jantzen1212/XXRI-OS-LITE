#!/bin/bash
# build.sh SYSROOT [OUT] - cross-build xxri-power-menu for the i686 target.
# Same recipe as the Store and Settings: host GTK3 headers, target GTK3 .so.
set -e
SYS="${1:?usage: build.sh SYSROOT [OUT]}"; OUT="${2:-xxri-power-menu}"
...
gcc -m32 -Os -w $(pkg-config --cflags gtk+-3.0) "$HERE/xxri-power-menu.c" $LF \
